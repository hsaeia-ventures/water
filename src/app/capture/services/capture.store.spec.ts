...
describe('CaptureStore', () => {
  ...
  let mockDbService: { getAll: any, put: any, delete: any, STORE_CAPTURES: string };

  const mockItems: CaptureItem[] = [
    { id: '1', text: 'older', createdAt: new Date('2024-01-01'), ghostTags: [], synced: false },
    { id: '2', text: 'newer', createdAt: new Date('2024-02-01'), ghostTags: [], synced: false }
  ];

  beforeEach(() => {
    mockDbService = {
      STORE_CAPTURES: 'captures',
  ...
  it('should auto-load items from DB on creation and sort them by date descending', async () => {
    ...
    expect(mockDbService.getAll).toHaveBeenCalledWith('captures');
    
    // Sort logic validation (newer should be first)
    expect(store.items()).toHaveLength(2);
    expect(store.items()[0].text).toBe('newer');
  ...
  it('should add a new capture to local DB and update signal array', async () => {
    ...
    expect(mockDbService.put).toHaveBeenCalledWith('captures', expect.objectContaining({
      text: 'Comprar leche',
      synced: false
    }));
    ...
    expect(store.items()[0].text).toBe('Comprar leche');
  ...
  it('should delete a capture from local DB and update signal array', async () => {
    ...
    expect(mockDbService.delete).toHaveBeenCalledWith('captures', '2');
