...
@Component({
  ...
  template: `
    <nav class="w-full h-full flex flex-col gap-2 py-6 px-4 border-r border-zinc-800/50 bg-zinc-950/30">
      ...
      <!-- Execute -->
      <div class="text-xs font-semibold text-zinc-500 uppercase tracking-wider mt-6 mb-2 px-3">Ejecución</div>
      <a routerLink="/execute" routerLinkActive="bg-zinc-800/50 text-white" class="flex items-center gap-3 px-3 py-2 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-900 transition-colors group">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-blue-500/60 group-hover:text-blue-400 transition-colors"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>
        <div class="flex flex-col">
          <span class="font-medium text-sm">Hacer</span>
          <span class="text-[10px] text-zinc-500 group-hover:text-zinc-400 transition-colors">Selección Tri-Focal</span>
        </div>
      </a>
